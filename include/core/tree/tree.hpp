...
class Tree {
  ...
  public:
    explicit Tree(const int root_id) : m_root_id(root_id) { add_node_with_id(root_id); }
    const TreeNode& get_node_by_id(int node_id) const;
    auto get_nodes() const {
        return m_nodeid_to_node_map |
               std::views::transform(
                   [](const auto& pair) -> const TreeNode& { return *pair.second; });
    }
...
class TreeNode {
  ...
  public:
    TreeNode(const int node_id, const Tree& owner) : m_id(node_id), m_owner(owner) {}
    [[nodiscard]] int get_id() const { return m_id; }
    [[nodiscard]] const TreeNode& get_parent() const { return m_owner.get_parent(m_id); }
