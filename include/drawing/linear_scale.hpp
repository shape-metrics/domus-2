...
class ScaleLinear {
 ...
 public:
  ScaleLinear(double domainMin, double domainMax, double rangeMin,
              double rangeMax, bool clamp = false)
  ...
  [[nodiscard]] double map(double x) const {
    ...
    if (m_clampEnabled) y = std::max(m_rangeMin, std::min(y, m_rangeMax));
    return y;
  }
