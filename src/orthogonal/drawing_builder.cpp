...
std::vector<int> path_in_class(const UndirectedSimpleGraph &graph,
                               const int from, const int to, const Shape &shape,
...
Cycle build_cycle_in_graph_from_cycle_in_ordering(
    const UndirectedSimpleGraph &graph, const Shape &shape,
    const Cycle &cycle_in_ordering, const DirectedSimpleGraph &ordering,
  ...
  for (size_t i = 0; i < cycle_in_ordering.size(); ++i) {
    ...
    const int next_next_class_id =
        cycle_in_ordering.next_of_node(next_class_id);
    const GraphEdge &next_edge =
        ordering.get_edge(next_class_id, next_next_class_id);
    const std::any &next_edge_label =
        ordering_edge_to_graph_edge.get_edge_any_label(next_edge.get_id());
    const int next_from =
        std::any_cast<std::pair<int, int>>(next_edge_label).first;
    if (to != next_from) {
      std::vector<int> path =
          path_in_class(graph, to, next_from, shape, go_horizontal);
      const auto end = static_cast<size_t>(static_cast<int>(path.size()) - 1);
      for (size_t j = 0; j < end; ++j) cycle.push_back(path[j]);
    }
  }
  return Cycle(cycle);
}
  ...
  for (const GraphNode *node : graph.get_nodes()) {
    ...
    const size_t side =
        node->get_degree() <= 4
            ? 25
            : static_cast<size_t>(
                  ceil(25 * sqrt(static_cast<double>(node->get_degree() - 3))));
...
void remove_useless_bends(UndirectedSimpleGraph &graph,
                          const GraphAttributes &attributes, Shape &shape) {
...
DrawingResult make_orthogonal_drawing_incremental(
    const UndirectedSimpleGraph &graph, std::vector<Cycle> &cycles);
...
std::optional<Cycle> check_if_metrics_exist(Shape &shape,
  ...
  if (cycle_x.has_value()) {
    return build_cycle_in_graph_from_cycle_in_ordering(
        graph, shape, cycle_x.value(), *ordering_x,
        ordering_x_edge_to_graph_edge, false);
  }
  if (cycle_y.has_value()) {
    return build_cycle_in_graph_from_cycle_in_ordering(
        graph, shape, cycle_y.value(), *ordering_y,
...
void build_nodes_positions(UndirectedSimpleGraph &graph,
                           GraphAttributes &attributes, Shape &shape);
...
void add_green_blue_nodes(UndirectedSimpleGraph &graph,
                          GraphAttributes &attributes, Shape &shape);

void make_shifts_overlapped_edges(UndirectedSimpleGraph &graph,
                                  GraphAttributes &attributes, Shape &shape);

void fix_negative_positions(const UndirectedSimpleGraph &graph,
...
DrawingResult make_orthogonal_drawing_incremental(
  ...
  std::optional<Cycle> cycle_to_add =
      check_if_metrics_exist(shape, *augmented_graph);
...
void find_inconsistencies(UndirectedSimpleGraph &graph, Shape &shape,
                          GraphAttributes &attributes);
...
auto find_edges_to_fix(const UndirectedSimpleGraph &graph, const Shape &shape,
                       const GraphAttributes &attributes) {
...
int get_other_neighbor_id(const UndirectedSimpleGraph &graph, const int node_id,
...
void fix_edge(UndirectedSimpleGraph &graph, int node_id, int other_node_id,
              Shape &shape, GraphAttributes &attributes, Direction direction) {
...
void add_green_blue_nodes(UndirectedSimpleGraph &graph,
  ...
  for (const GraphNode *node : nodes) {
    ...
    for (auto [from_id, to_id] : edges_to_add) graph.add_edge(from_id, to_id);
...
void fix_inconsistency(const Cycle &cycle, GraphAttributes &attributes,
                       const UndirectedSimpleGraph &graph, Shape &shape,
...
void shifting_order(const int node_id, UndirectedSimpleGraph &graph,
                    Shape &shape, std::vector<int> &nodes_at_direction,
                    GraphAttributes &attributes,
  ...
  std::sort(
      nodes_at_direction.begin(), nodes_at_direction.end(), [&](int a, int b) {
        if (attributes.get_node_color(a) == Color::BLACK) {
          const int b_other_neighbor_id =
              get_other_neighbor_id(graph, b, node_id);
          return shape.get_direction(b, b_other_neighbor_id) ==
                 increasing_direction;
        }
        if (attributes.get_node_color(b) == Color::BLACK) {
          const int a_other_neighbor_id =
              get_other_neighbor_id(graph, a, node_id);
...
void make_shifts(const int node_id, UndirectedSimpleGraph &graph, Shape &shape,
                 GraphAttributes &attributes, std::vector<int> &right_nodes,
                 const Axis axis, const Direction increasing_direction,
                 const Color color) {
  auto position_function =
      axis == Axis::X ? [](const GraphAttributes &a,
                           const int id) { return a.get_position_x(id); }
                      : [](const GraphAttributes &a, const int id) {
                          return a.get_position_y(id);
                        };
  ...
  const auto position_function_other =
      axis == Axis::X ? [](const GraphAttributes &a,
                           const int id) { return a.get_position_y(id); }
                      : [](const GraphAttributes &a, const int id) {
                          return a.get_position_x(id);
                        };
  const auto change_position_other =
      axis == Axis::X ? [](GraphAttributes &a, const int id,
                           const int value) { a.change_position_y(id, value); }
                      : [](GraphAttributes &a, const int id, const int value) {
                          a.change_position_x(id, value);
                        };
  const size_t index_of_fixed_node =
      find_fixed_index_node(attributes, right_nodes);
  const int initial_position = position_function_other(attributes, node_id);
  for (const GraphNode *node : graph.get_nodes()) {
    const int old_position_y =
        position_function_other(attributes, node->get_id());
    ...
    if (old_position_y < initial_position) {
      const int new_position_y =
          old_position_y - 5 * static_cast<int>(index_of_fixed_node);
      change_position_other(attributes, node->get_id(), new_position_y);
    }
  }
  for (size_t i = 0; i < right_nodes.size(); ++i) {
    ...
    const int shift =
        (static_cast<int>(i) - static_cast<int>(index_of_fixed_node)) * 5;
...
void make_shifts_overlapped_edges(UndirectedSimpleGraph &graph,
  ...
  for (const GraphNode *node : nodes) {
    ...
    make_shifts(node_id, graph, shape, attributes,
                nodes_to_sort[Direction::RIGHT], Axis::X, Direction::UP,
                Color::GREEN);
    make_shifts(node_id, graph, shape, attributes, nodes_to_sort[Direction::UP],
                Axis::Y, Direction::RIGHT, Color::BLUE);
    make_shifts(node_id, graph, shape, attributes,
                nodes_to_sort[Direction::LEFT], Axis::X, Direction::UP,
                Color::GREEN_DARK);
    make_shifts(node_id, graph, shape, attributes,
                nodes_to_sort[Direction::DOWN], Axis::Y, Direction::RIGHT,
                Color::BLUE_DARK);
  }
}
...
void fix_negative_positions(const UndirectedSimpleGraph &graph,
  ...
  if (min_x < 0) {
    for (const int node_id : nodes_ids)
      attributes.change_position_x(node_id,
                                   attributes.get_position_x(node_id) - min_x);
  }
  if (min_y < 0) {
    for (const int node_id : nodes_ids) {
      attributes.change_position_y(node_id,
                                   attributes.get_position_y(node_id) - min_y);
    }
  }
}