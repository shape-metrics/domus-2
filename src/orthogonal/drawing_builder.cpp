...
Cycle build_cycle_in_graph_from_cycle_in_ordering(
    ...
    const Graph& ordering, const GraphAttributes& ordering_edge_to_graph_edge,
...
void remove_useless_bends(Graph& graph, const GraphAttributes& attributes,
                          Shape& shape) {
...
int min_coordinate(
  ...
  int min_c = INT_MAX;
  for (const auto& coord : coordinate_to_nodes | std::views::keys)
...
void fix_inconsistency(const Cycle& cycle, GraphAttributes& attributes,
                       const Graph& graph, Shape& shape, Color color_to_find) {
...
void make_shifts(const int node_id, Graph& graph, Shape& shape,
                 GraphAttributes& attributes, std::vector<int>& right_nodes,
                 const Axis axis, Direction increasing_direction,
                 const Color color) {
