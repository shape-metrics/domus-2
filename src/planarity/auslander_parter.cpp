...
Embedding merge_biconnected_components(const UndirectedSimpleGraph& graph,
    ...
    for (size_t i = 0; i < biconnected_components.get_components().size(); ++i) {
        ...
        const UndirectedSimpleGraph& component = *biconnected_components.get_components()[i];
        for (const int node_id : component.get_nodes_ids()) {
...
Embedding base_case_graph(const UndirectedSimpleGraph& graph) {
    Embedding embedding(graph);
    for (const GraphNode* node : graph.get_nodes())
        for (const GraphEdge& edge : node->get_edges())
            embedding.add_edge(node->get_id(), edge.get_to_id());
    return embedding;
}

Embedding base_case_component(const UndirectedSimpleGraph& component, const Cycle& cycle) {
...
Cycle change_cycle_with_path(const Cycle& cycle,
...
auto compute_min_and_max_segments_attachments(const std::vector<Segment>& segments,
                                              const Cycle& cycle) {
...
std::vector<bool> are_embeddings_inside_clockwise_cycle(const Cycle& cycle,
                                                        const std::vector<Embedding>& embeddings,
    ...
    for (size_t i = 0; i < segments.size(); ++i) {
        ...
        const Embedding& embedding = embeddings[i];
        const int attachment_id = *segment.get_attachments().begin(); // any attachment is good
...
std::vector<size_t> compute_order(const std::vector<size_t>& segments_indexes,
...
void add_middle_edges(const Embedding& embedding,
...
void add_edges_incident_to_cycle(const std::vector<Segment>& segments,
                                 const Cycle& cycle,
                                 const std::vector<Embedding>& embeddings,
    ...
    for (size_t cycle_node_position = 0; cycle_node_position < cycle.size();
        ...
        std::vector<size_t> inside_order = compute_order(inside_segments,
        ...
        std::vector<size_t> outside_order = compute_order(outside_segments,
                                                          segments_min_attachment,
                                                          segments_max_attachment,
                                                          segments,
                                                          cycle_node_position);
...
void add_edges_not_incident_to_cycle(const std::vector<Segment>& segments,
                                     Embedding& output,
                                     const Cycle& cycle,
                                     const std::vector<Embedding>& embeddings,
                                     const std::vector<bool>& is_embedding_inside,
    ...
    for (size_t i = 0; i < segments.size(); ++i) {
        const Segment& segment = segments[i];
        const Embedding& embedding = embeddings[i];
        for (const int node_id : segment.get_segment().get_nodes_ids()) {
...
Embedding merge_segments_embeddings(const UndirectedSimpleGraph& component,
                                    const Cycle& cycle,
                                    const std::vector<Embedding>& embeddings,
                                    const std::vector<Segment>& segments,
                                    const std::unordered_map<int, bool>& is_segment_inside) {
    ...
    add_edges_incident_to_cycle(segments,
                                cycle,
                                embeddings,
                                is_segment_inside,
                                output,
                                ...
                                segments_max_attachment,
                                is_embedding_inside);
    add_edges_not_incident_to_cycle(segments,
                                    output,
                                    cycle,
                                    embeddings,
                                    is_embedding_inside,
...
std::optional<Embedding> embed_biconnected_component(const UndirectedSimpleGraph& component) {
    const std::optional<Cycle> cycle = find_a_cycle_in_graph(component);
    if (cycle.has_value())
