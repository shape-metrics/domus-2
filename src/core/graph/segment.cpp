...
void dfs_find_segments(const UndirectedSimpleGraph &graph, const int node_id,
  ...
  for (const GraphEdge &edge : node.get_edges()) {
    ...
    if (!is_node_visited.contains(neighbor_id))
      dfs_find_segments(graph, edge.get_to_id(), is_node_visited,
                        nodes_in_segment, cycle, edges_in_segment);
  }
}
...
void find_segments(const UndirectedSimpleGraph &graph, const Cycle &cycle,
                   std::vector<Segment> &segments) {
  std::unordered_set<int> visited;
  for (const int node_id : graph.get_nodes_ids())
    if (cycle.has_node(node_id)) visited.insert(node_id);
  for (const int node_id : graph.get_nodes_ids()) {
...
void find_chords(const UndirectedSimpleGraph &graph, const Cycle &cycle,
                 std::vector<Segment> &segments) {
  for (const int node_id : cycle) {
    const GraphNode &node = graph.get_node_by_id(node_id);
    for (const GraphEdge &edge : node.get_edges()) {
