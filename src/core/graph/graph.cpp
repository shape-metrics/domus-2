...
std::string GraphEdge::to_string() const {
  return "Edge[" + std::to_string(m_from_id) + " -> " +
...
int Graph::add_edge(const int from_id, const int to_id) {
  ...
  m_edgeid_to_edge_map[edge_id] =
      std::make_unique<GraphEdge>(edge_id, from_node, to_node);
...
void Graph::remove_edge(const int edge_id) {
  ...
  m_edge_to_edgeids.at({edge.get_from_id(), edge.get_to_id()})
      .erase(edge.get_id());
...
void Graph::remove_node(const int node_id) {
  ...
  for (const int edge_id : edges_to_remove) remove_edge(edge_id);
...
size_t Graph::get_edge_count(const int from_id, const int to_id) const {
  if (!m_edge_to_edgeids.contains({from_id, to_id})) return 0;
  return m_edge_to_edgeids.at({from_id, to_id}).size();
}

const std::unordered_set<int> &Graph::get_edgeids(const int from_id,
                                                  const int to_id) const {
  return m_edge_to_edgeids.at({from_id, to_id});
}

bool Graph::has_node(const int node_id) const {
  return m_nodeid_to_node_map.contains(node_id);
}

std::string Graph::to_string() const {
  std::string result = "Graph:\n";
  for (const GraphNode *node : get_nodes()) result += node->to_string() + "\n";
  return result;
}
...
GraphNode::GraphNode(const int id, const Graph &owner)
    : m_id(id), m_graph_owner(owner) {}
...
std::vector<GraphEdge> DirectedMultiGraph::get_edges_of_node(
    const int node_id) const {
  if (!has_node(node_id))
    throw std::runtime_error(
        "DirectedMultiGraph::get_edges_of_node: node not found");
  std::vector<GraphEdge> edges;
...
std::vector<GraphEdge> DirectedMultiGraph::get_in_edges_of_node(
    const int node_id) const {
  if (!has_node(node_id))
    throw std::runtime_error(
        "DirectedMultiGraph::get_in_edges_of_node: node not found");
  std::vector<GraphEdge> edges;
...
int DirectedSimpleGraph::add_edge(const int from_id, const int to_id) {
  if (has_edge(from_id, to_id))
    throw std::runtime_error(
        "DirectedSimpleGraph::add_edge: edge already exists");
  return DirectedMultiGraph::add_edge(from_id, to_id);
}

void DirectedSimpleGraph::remove_edge(int edge_id) {
  DirectedMultiGraph::remove_edge(edge_id);
}
...
GraphEdge DirectedSimpleGraph::get_edge(const int from_id,
                                        const int to_id) const {
...
std::vector<GraphEdge> UndirectedMultiGraph::get_edges_of_node(
    const int node_id) const {
  std::vector<GraphEdge> edges = Graph::get_edges_of_node(node_id);
  for (GraphEdge &edge : edges)
    if (edge.get_to_id() == node_id) edge.reverse();
  return edges;
}
...
int UndirectedSimpleGraph::add_edge(const int from_id, const int to_id) {
  if (has_edge(from_id, to_id))
    throw std::runtime_error(
        "UndirectedSimpleGraph::add_edge: edge already exists");
  return UndirectedMultiGraph::add_edge(from_id, to_id);
}

void UndirectedSimpleGraph::remove_edge(int edge_id) {
...
int UndirectedSimpleGraph::get_edge_id(const int from_id,
                                       const int to_id) const {
  const auto &ids_1 = get_edgeids(from_id, to_id);
  const auto &ids_2 = get_edgeids(from_id, to_id);
  if (ids_1.size() == 1) return *ids_1.begin();
  if (ids_2.size() == 1) return *ids_2.begin();
  throw std::runtime_error(
      "UndirectedSimpleGraph::get_edge_id: edge not found");
}
